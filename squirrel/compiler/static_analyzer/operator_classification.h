...
static bool isSuspiciousNeighborOfNullCoalescing(TreeOp op) {
...
static bool isSuspiciousTernaryConditionOp(TreeOp op) {
...
static bool isSuspiciousSameOperandsBinaryOp(TreeOp op) {
...
static bool isBlockTerminatorStatement(TreeOp op) {
  return op == TO_RETURN || op == TO_THROW || op == TO_BREAK || op == TO_CONTINUE;
}

static bool isBooleanBinaryResultOperator(TreeOp op) {
  return op == TO_NE || op == TO_EQ || (TO_GE <= op && op <= TO_IN);
}

static bool isBooleanResultOperator(TreeOp op) {
  return isBooleanBinaryResultOperator(op) || op == TO_NOT;
}

static bool isArithOperator(TreeOp op) {
...
static bool isDivOperator(TreeOp op) {
  return op == TO_DIV || op == TO_MOD || op == TO_DIVEQ || op == TO_MODEQ;
}

static bool isPureArithOperator(TreeOp op) {
  return (TO_USHR <= op && op <= TO_SUB) || (TO_PLUSEQ <= op && op <= TO_MODEQ);
}

static bool isRelationOperator(TreeOp op) {
  return TO_3CMP <= op && op <= TO_LT;
}

static bool isBoolRelationOperator(TreeOp op) {
  return TO_GE <= op && op <= TO_LT;
}

static bool isBitwiseOperator(TreeOp op) {
  return op == TO_OR || op == TO_AND || op == TO_XOR;
}

static bool isBoolCompareOperator(TreeOp op) {
  return op == TO_NE || op == TO_EQ || isBoolRelationOperator(op);
}

static bool isCompareOperator(TreeOp op) {
  return TO_NE <= op && op <= TO_LT;
}

static bool isShiftOperator(TreeOp op) {
  return op == TO_SHL || op == TO_SHR || op == TO_USHR;
}

static bool isHigherShiftPriority(TreeOp op) {
  return TO_MUL <= op && op <= TO_SUB;
}

static bool isAssignOp(TreeOp op) {
